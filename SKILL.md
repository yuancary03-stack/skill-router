...
Route by capability, not by a fixed personal skill list. Choose the smallest useful set of available skills for the task, load only those skills, then continue with the work.
...
5. Read or invoke only the selected skill bodies.
...
For other clients, inspect their skill/rule directories in the same spirit: read names and descriptions first, then load only the chosen file.
